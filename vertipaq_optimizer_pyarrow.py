...
BIT_SAVINGS_THRESHOLD = 64.0     # Minimum bit savings to continue
...
FREQUENCY_THRESHOLD_FACTOR = 0.1 # Factor for frequency threshold: freq < cols² * this
...
class VertiPaqOptimizer:
    ...
    def _compress_segment(
        ...
        buckets = []
        heapq.heappush(buckets, _Bucket(0, segment_size - 1))
        ...
        while buckets:
            ...
            best_choice = None
            num_cols = len(columns)  # For frequency threshold calculation
            ...
            for col_idx in range(num_cols):
                ...
                if cluster_count[col_idx] > 0 and segment_size / cluster_count[col_idx] < 64.0:
                    print(f"    [Segment {segment_idx}] Column {col_idx} avg cluster size too small, skipping")
                ...
                if max_value is None:
                    bucket.mark_column_done(col_idx)
                    continue
                # NEW: Frequency vs column count squared check (CRITICAL!)
                # This is a diminishing returns check that scales with problem complexity
                # Formula: if frequency < columnCount² * 0.1 → mark column done
                # With 62 columns: threshold = 384.4
                freq_threshold = (num_cols +1)* (num_cols +1)* FREQUENCY_THRESHOLD_FACTOR
...
class _Bucket:
    ...
    def __lt__(self, other):
        """
        Comparison for max-heap ordering.
        
        Priority:
        1. Larger buckets first (higher compression potential)
        2. Earlier start index as tiebreaker (deterministic ordering)
        
        Note: Uses reversed comparison for max-heap with heapq (min-heap)
        """
        if self.size() == other.size():
            return self.start < other.start  # Earlier start wins
        return self.size() > other.size()  # Larger size wins
...
class _ColumnStats:
    ...
    def __init__(self, column: pa.ChunkedArray):
        ...
        if self.cardinality <= 1:
            ...
            self.bits_per_value = math.ceil(math.log2(self.cardinality))
